 ...
 */
contract CrossChainAggregator is DIExecutable, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // DI gateway contract
    IDIGateway public immutable diGateway;
    ...
    ) DIExecutable(gateway_) Ownable() {
        diGateway = IDIGateway(gateway_);
    ...
    function crossChainSwap(
        ...
        // Remove IXFI token check as we now support multiple bridged tokens
    ...
    function _executeWithToken(
        ...
        try this._performDestinationSwap(
            ...
            // Send tokens directly to user as fallback
            // Note: This would need to be updated based on actual token contract
    ...
    function _performDestinationSwap(
        ...
        // Get token from registry based on symbol
        // This is simplified - in practice you'd get the token address from the registry
        // For now, assume we have the bridged token address
        address bridgedToken = address(0); // Would get from token registry
        require(bridgedToken != address(0), "Token not found");

        // Approve router to spend bridged token
        IERC20(bridgedToken).approve(router, ixfiAmount);
