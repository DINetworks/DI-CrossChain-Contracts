...
contract SwapCalldataUsageExample {
    ...
    constructor(address _calldataGenerator, address _diGateway) {
        calldataGenerator = SwapCalldataGenerator(_calldataGenerator);
        diGateway = IDIGateway(_diGateway);
    }
    ...
    function executeCrossChainSwap(
        ...
        // Approve DI Gateway to spend tokens
        IERC20(tokenIn).approve(address(diGateway), amountIn);
    ...
    function _getChainId(string memory chainName) internal pure returns (uint256 chainId) {
        ...
        if (nameHash == keccak256(bytes("crossfi"))) return 4157;
