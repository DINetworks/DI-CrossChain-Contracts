...
describe("IXFI GMP Protocol Complete Test", function () {
    ...
    beforeEach(async function () {
        ...
        // Set up relayer
        await ixfi.addWhitelistedRelayer(relayer.address);
    ...
    describe("GMP Cross-Chain Contract Calls", function () {
        ...
        it("Should execute a complete cross-chain contract call with token flow", async function () {
            ...
            const tx2 = await ixfi.connect(relayer).execute(commandId, [command], '0x00');
            await tx2.wait();
    ...
    describe("GMP Token Transfer", function () {
        it("Should execute a complete cross-chain token transfer", async function () {
            ...
            const tx2 = await ixfi.connect(relayer).execute(commandId, commands, '0x00');
            await tx2.wait();
    ...
    describe("Relayer Management", function () {
        it("Should manage relayer permissions correctly", async function () {
            ...
            // Test adding relayer
            await ixfi.addWhitelistedRelayer(user1.address);
            expect(await ixfi.isWhitelistedRelayer(user1.address)).to.be.true;
            console.log("✅ Relayer added successfully");

            // Test removing relayer
            await ixfi.removeWhitelistedRelayer(user1.address);
            expect(await ixfi.isWhitelistedRelayer(user1.address)).to.be.false;
            ...
            const commands = [{
                ...
                data: ethers.AbiCoder.defaultAbiCoder().encode(
                    ['address', 'uint256', 'string'],
                    [user2.address, 100, "IXFI"]
                )
            }];

            await expect(
                ixfi.connect(user1).execute(commandId, commands, '0x00')
